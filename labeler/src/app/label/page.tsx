...
export default function Home() {
  ...
  const [customSvUrl, setCustomSvUrl] = useState("");
    ...
    if (!panoRef.current) {
      ...
      panoRef.current.addListener("pano_changed", () => {
        ...
        svService.getPanorama({ pano: povRef.current.pano }, (data) => {
          if (data?.imageDate) {
            povRef.current.image_date = data.imageDate;
          }
        });
      });
  ...
  const handleLabelRef = useRef<((label: StatusLabels) => void) | null>(null);

  const handleLabel = useCallback(
    async (label: StatusLabels) => {
      const stop = stops[stopIndex];
      if (!stop) return;
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      // Don't trigger when typing in the notes input
      if ((e.target as HTMLElement).tagName === "INPUT") return;
      const fn = handleLabelRef.current;
      if (!fn) return;
      if (e.key === "a" || e.key === "A") fn("blocked");
      else if (e.key === "d" || e.key === "D") fn("not_blocked");
    };
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="px-4 py-2 bg-gray-900 text-sm flex items-center gap-3">
        <div>
        ...
        <a
          href={`https://www.google.com/maps?q=${stop.snapped_lat},${stop.snapped_lon}&z=18`}
          target="_blank"
          rel="noopener noreferrer"
          className="ml-auto text-xs bg-blue-600 hover:bg-blue-500 text-white px-2 py-1 rounded transition-colors"
        >
          Google Maps
        </a>
        <form
          onSubmit={(e) => {
            ...
            if (panoMatch) {
              ...
              const heading = parseFloat(coordMatch[4]);
        ...
        >
          <input
            type="text"
            placeholder="Paste Street View URL..."
            value={customSvUrl}
            onChange={(e) => setCustomSvUrl(e.target.value)}
            className="bg-gray-800 border border-gray-700 rounded px-2 py-1 text-xs w-64 focus:outline-none focus:border-blue-500"
          />
          <button
            type="submit"
            className="text-xs bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded transition-colors cursor-pointer"
          >
            Go
          </button>
        </form>
